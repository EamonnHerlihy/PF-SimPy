...
def asset_trajectory(env, asset_id, results, records):
...
def run_simulation(num_assets):
    ...
    for asset_id in range(1, num_assets + 1):
        env.process(asset_trajectory(env, asset_id, results, records))
    env.run()
    return results, records

if __name__ == "__main__":
    results, records = run_simulation(SIMULATION_DETAILS["num_assets"])
    print("\nSimulation Results:")
